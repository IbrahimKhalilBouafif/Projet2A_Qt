...
Courrier::Courrier(int id,int expediteur,QString destinataire,QString statut)
{
    this->id=id;
...
bool Courrier::ajouter()
{
    ...
    query.prepare("INSERT INTO COURRIER (id,expediteur,destinataire,statut)VALUES(:id,:expediteur,:destinataire,:statut)");
...
QSqlQueryModel * Courrier::afficher(QString search)
{

    QSqlQueryModel * model= new QSqlQueryModel();
    QString qry="select * from courrier where id like '%"+search+"%' or expediteur like '%"+search+"%' or destinataire like '%"+search+"%' or statut like '%"+search+"%'";
    qDebug()<<qry;
...
bool Courrier::modifier(int idd)
{
    QSqlQuery query;
    query.prepare("UPDATE COURRIER SET expediteur=:expediteur,destinataire=:destinataire,statut=:statut WHERE id=:idd");
...
QSqlQueryModel * Courrier::tri()
{
    QSqlQueryModel * model= new QSqlQueryModel();
    model->setQuery("SELECT * FROM courrier ORDER BY id");
    return model;

}
void Courrier::pdf(int idd)
{
    if(idd!=-1){
    QSqlQuery qry;
    qry.prepare("SELECT * FROM COURRIER WHERE ID=:idd");
    qry.bindValue(":idd",idd);
    qry.exec();
     system("del /f D:\\template\\example_courrier.pdf");
     system("del /f D:\\template\\Data_courrier.html");
        QFile filein(QString("D:\\template\\courrier.html")) ;
        ...
        QString filename="D:\\template\\Data_courrier.html";
        QFile file( filename );
        ...
        system("D:\\template\\wkhtmltopdf.exe -T 0 -B 0 --page-width 210mm --page-height 297mm \"D:\\template\\Data_courrier.html\" D:\\template\\example_courrier.pdf");
        // system("del /f C:\\sqlite\\test12.pdf");
        QDesktopServices::openUrl(QUrl("file:///D:/template/example_courrier.pdf"));

    }
    else{
        QSqlQuery qry;
        qry.prepare("SELECT * FROM COURRIER ");
        qry.exec();
         system("del /f D:\\template\\example_courrier.pdf");
         system("del /f D:\\template\\Data_courrier.html");
            QFile filein(QString("D:\\template\\courrier.html")) ;
...
void Courrier::stat(QCustomPlot *customPlot)
{
 ...
 //   regen->moveAbove(nuclear);
    ...
    query.prepare("SELECT COUNT(DISTINCT id) FROM courrier where statut = 'En transit'");
    query.exec();
    int trans;
    while(query.next())
    {
    ...
    query.prepare("SELECT COUNT(DISTINCT id) FROM courrier where statut = 'ReÃ§u'");
    query.exec();
    int recu;
    while(query.next())
    {
    ...
    customPlot->yAxis->setLabel("Nombre des courriers en transit et reçu");
