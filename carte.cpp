...
bool Carte::ajouter(Carte Ca)
{
    ...
    query.prepare("INSERT INTO TABCARTE (REF,CINCLIENT,NOMBREDESPOINTS) "
...
model->setQuery("select * from TABCARTE");
...
query.prepare("Delete from tabcarte where ref = :ref ");
...
bool Carte::modifier(QString REF,Carte Ca)
{
    QSqlQuery query;
       ...
       query.prepare("UPDATE TABCARTE SET  CINCLIENT =:cinClient ,NOMBREDESPOINTS =:nombredespoints WHERE REF =:ref ");
       query.bindValue(":ref", Ca.get_ref());
...
QSqlQueryModel *model = new QSqlQueryModel();
q->prepare("SELECT * FROM TABCARTE ORDER BY NOMBREDESPOINTS");
q->exec();
model->setQuery(*q);
return model;
}
...
model->setQuery("SELECT * FROM TABCARTE WHERE (ref LIKE '"+id+"%')");
model->setHeaderData(0, Qt::Horizontal, QObject::tr("ref"));
model->setHeaderData(1, Qt::Horizontal, QObject::tr("cinClient"));
model->setHeaderData(2, Qt::Horizontal, QObject::tr("nombredespoints"));
