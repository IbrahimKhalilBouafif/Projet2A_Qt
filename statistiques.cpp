...
int i =0;
 ...
 fossil->setBrush(QColor(0, 0, 255));
   ...
   /* QCPBars *regen = new QCPBars(ui->widget->xAxis, ui->widget->yAxis);
    ...
    query.prepare("SELECT DISTINCT DATE_DEBUT FROM evenement");
    query.exec();
    ...
    while(query.next())
    {
     ...
     query1.prepare("SELECT matricule,sum(NOM) FROM vehicule where marque = '"+query.value(0).toString()+"' Group By DATE_DEBUT");
     query1.exec();
     while (query1.next()) {
  ...
  //  labels << "5000" << "10000" << "15000" ;
    ...
    ui->widget->yAxis->setLabel("nombre d'evenement par date de début");
